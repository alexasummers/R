#1 True -- The model that inclufes all the variables will always have the largest R^2
...
#3 True -- BIC method adds penalty to RSS for the nunber of variables in the model || log(n)
...
#8 True -- BIC takes log(n) instead of constant 2. When log(n) > 2, it gives heavier penaltly with many variables.
...
#10 True -- In lasso, when lambda = 0, we get the lienar least squares estimates.
...
#13 True -- As we increase lambda from 0, the training RSS will stadily increase.
#14 True -- As we increase lambda from 0, the test RSS will decerase initially and then eventually start increasing in a U shape.
...
#16 True -- As we icnrease lambda from 0, the bias will steadily increase.
